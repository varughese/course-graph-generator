...
SUBJECTS = ['ADMJ','ADMPS', 'AFRCNA', 'AFROTC', 'ANTH', 'ARABIC', 'ARTSC', 'ASL', 'ASTRON', 'ATHLTR', 'BACC', 'BCHS',
			...
			'CHE', 'CHEM', 'CHIN', 'CLASS', 'CLRES', 'CMME', 'CMMUSIC', 'CMPBIO', 'CMPINF', 'COE', 'COEA',
			'COMMRC', 'CS', 'CSD', 'DENHYG', 'DENT', 'DIASCI', 'DMED', 'DSANE', 'DUPOSC', 'EAS', 'ECE', 'ECON',
...
class PittSection:
	...
	def __extract_data_from_div_section(tag: Tag) -> str:
		return str(tag.find('div', {'class': 'pull-right'}).next_element.next_element.next_element)
	...
	def extra_details(self) -> Dict[str, Any]:
		...
		try:
			...
			return None
		self._extra = {
			'units': self.__extract_data_from_div_section(data[3]),
			'description': ''
		}
		try:
			self._extra['description'] = self.__extract_data_from_div_section(data[5])
		except:
			print('caught no description on ' + self.parent_subject.subject + self.parent_course.number) 

		if 'Enrollment Requirements' in data[6].text:
			self._extra['preq'] = ''
			...
			self._extra['preq'] = self.__extract_data_from_div_section(data[6])

			if 'Class Attributes' in data[7].text:
				self._extra['class_attributes'] = self.__extract_data_from_div_section(data[7]).split('\n')
		elif 'Class Attributes' in data[6].text:
