...
parser.add_argument('--reparse_prereqs', help='Recompiler the PegJS parser and then reparse the json', default=False, action="store_true")
...
if args.reparse_prereqs or not os.path.isfile("./parser/parser.js"):
	print("Running PegJS Parser")
	os.system("yarn run pegjsparse")

if args.reparse_prereqs or not os.path.isfile(course_parsed_raw_path):
...
# I just stopped working that hard on this

if fixpreqs != '':
	print("Fixing preqrequisites")
	if not args.reparse_prereqs and os.path.isfile(course_parsed_fixing_path):
